...
#ifndef NVE_RAINDROPS_COUNT
    #define NVE_RAINDROPS_COUNT 256
#endif

#ifndef NVE_RAINDROPS_RES_REDUCTION
    #define NVE_RAINDROPS_RES_REDUCTION 2
#endif
...
#define RES_X (1920/NVE_RAINDROPS_RES_REDUCTION)
#define RES_Y (1080/NVE_RAINDROPS_RES_REDUCTION)
...
void CS_Simulate(uint3 tid : SV_DispatchThreadID) {
    ...
    if (idx >= NVE_RAINDROPS_COUNT || tid.y >= 3 || tid.y < 0 || RainValue <= 0) return;
    ...
    if ((life <= 0 || pos.x < 0.0-DROPLET_SIZE || pos.x > 1.0+DROPLET_SIZE || pos.y < 0.0-DROPLET_SIZE || pos.y > 1.0+DROPLET_SIZE) && (idx < (RainValue*NVE_RAINDROPS_COUNT))) {
...
float4 PS_Raindrops(float4 pos : SV_POSITION, float2 uv : TEXCOORD) : SV_Target {
    ...
    [loop]
    for (uint i = 0; i < int(NVE_RAINDROPS_COUNT*RainValue); i++) {
        float4 droplet = tex2Dlod(DropletBufferSampler, float4((i + 0.5) / NVE_RAINDROPS_COUNT, 0.25, 0, 0));
        float4 droplet2 = tex2Dlod(DropletBufferSampler, float4((i + 0.5) / NVE_RAINDROPS_COUNT, 0.75, 0, 0));

        if (droplet.z <= 0) continue;
...
float4 PS_Blur(float4 pos : SV_POSITION, float2 uv : TEXCOORD) : SV_Target {
    ...
    [branch]
    if(NVE_RAINDROPS_COUNT <= 0 || RainValue <= 0){
...
technique Raindrops < ui_label = "NaturalVision Evolved: Raindrops"; hidden=false;>{
    ...
    pass Simulate {
        ComputeShader = CS_Simulate;
        DispatchSizeX = NVE_RAINDROPS_COUNT / (THREADS_X * 2);
        DispatchSizeY = 1;
    }
