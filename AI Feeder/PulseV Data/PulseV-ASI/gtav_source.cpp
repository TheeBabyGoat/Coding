...
namespace GTAV
{
    ...
    const bool GTAVSource::get_depth_reversed() { return true; }

    const std::string_view GTAVSource::get_region_name(int region) { return GTAV::get_region_name(region); }
    const std::string_view GTAVSource::get_weather_name(int weather) { return GTAV::get_weather_name(weather); }

    const UInt2 GTAVSource::get_resolution()
    {
        int res_x = 0, res_y = 0;
        GET_ACTUAL_SCREEN_RESOLUTION(&res_x, &res_y);
        return { static_cast<unsigned int>(res_x), static_cast<unsigned int>(res_y) };
    }
    ...
    const float GTAVSource::get_cam_fov() { return GET_FINAL_RENDERED_CAM_FOV(); }
    const float GTAVSource::get_cam_near_clip() { return GET_FINAL_RENDERED_CAM_NEAR_CLIP(); }
    const float GTAVSource::get_cam_far_clip() { return GET_FINAL_RENDERED_CAM_FAR_CLIP(); }

    const Float3 GTAVSource::get_wind_dir() { return GET_WIND_DIRECTION(); }
    const float GTAVSource::get_wind_speed() { return GET_WIND_SPEED(); }
    ...
    const float GTAVSource::get_time()
    {
        ...
        return std::clamp(time_h + (time_m / 60.0f) + (time_s / 3600.0f), 0.0f, 24.0f);
    }
    ...
    const int GTAVSource::get_weather_from() { return get_weather_from_hash(from_weather_hash); }
    const int GTAVSource::get_weather_to() { return get_weather_from_hash(to_weather_hash); }
    const float GTAVSource::get_weather_transition() { return weather_transition; }
    ...
    void GTAVSource::load_timecycle() { timecycle.load(); }
    void GTAVSource::wait(DWORD time) { scriptWait(time); }
    void GTAVSource::update() { GET_CURR_WEATHER_STATE(&from_weather_hash, &to_weather_hash, &weather_transition); }

    void GTAVSource::register_script(HMODULE hModule, void(*entry)()) { scriptRegister(hModule, entry); }
    void GTAVSource::unregister_script(HMODULE hModule) { scriptUnregister(hModule); }
}
#endif
