...
------------------------------------------------------------------------------
timecycle.cpp
Time-Based Weather and Lighting Interpolation
Part of the PulseV ASI project


Overview:
---------
Implements logic for evaluating and interpolating weather and lighting
parameters over time-of-day curves.


Components:
-----------
• TimeCycle::Variable:
- Stores a time-of-day curve.
- Supports evaluation at a specific time.
- Supports interpolation between two curves.


• TimeCycle::ColorVariable:
- Composed of four Variable curves for RGBA.
- Supports value sampling and smooth blending.


• TimeCycle::WeatherCycle:
- Stores mapped float and color variables.
- Generates WeatherFrame snapshots from time or interpolation.


• TimeCycle:
- Manages the full regional weather map.
- Can generate WeatherFrame from region-to-region blend.
- Loads XML-based definitions from disk.


• Utility:
- Path resolution for default and override XMLs.
- XML preprocessing (merging text fragments, stripping comments).


Notes:
------
• Variables wrap around 24h (interpolated).
• Safe fallback to DEFAULT_VALUE for missing curves.
• XML documents are modified in-memory for consistent parsing.
• All core classes return copies for value safety.
