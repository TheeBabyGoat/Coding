...
// Resolve the ASI base path for timecycle XMLs
static std::string get_asi_base_path() {
    char path[MAX_PATH];
    GetModuleFileNameA(GetModuleHandle(NULL), path, MAX_PATH);
    std::filesystem::path base(path);
    return base.parent_path().string() + "\\PulseV\\timecycle\\";
}
...
// Merge consecutive XML text nodes & strip comments
...
{
    const std::string path = get_asi_base_path() + filename + ".xml";
    if (!std::filesystem::exists(path)) {
        log_error("TimeCycle file not found: " + path);
        return false;
    }

    document->LoadFile(path.c_str());
    strip_xml_comments(document);
    log_info("Loaded TimeCycle XML: " + path);
    return true;
}

void TimeCycle::load() {
    // Load all necessary timecycle files
    int loaded_files = 0;
    for (auto& it : timecycles) {
        tinyxml2::XMLDocument doc;
        if (get_xml_doc_from_filename(it.first.to_string(), &doc)) {
            loaded_files++;
    ...
    if (loaded_files == 0) {
        log_info("[WARNING] No TimeCycle XML files loaded. Using default values.");
    }
}
