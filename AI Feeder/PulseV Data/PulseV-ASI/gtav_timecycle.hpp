...
------------------------------------------------------------------------------
	gtav_timecycle.hpp
	GTA V Timecycle and Weather Mapping
	Part of the PulseV ASI project

	Overview:
	---------
	Declares GTAVTimeCycle, a specialization of the TimeCycle system for GTA V.
	Provides mapping between weather/region identifiers, XML timecycle files,
	and the internal timecycle representation used by the engine.

	Key Points:
	-----------
	  • Enumerations:
		  - WeatherType (CLEAR, RAIN, SNOW, etc. – 15 total).
		  - Region (GLOBAL, URBAN).
	  • Static tables:
		  - REGION_HASH_MAP / REGION_NAMES.
		  - WEATHER_HASH_MAP / WEATHER_NAMES / WEATHER_TIMECYCLE_FILES.
		  - TIME_FRAMES (13 time-of-day breakpoints).
	  • Variable Mappings:
		  - FLOAT_VARIABLES and COLOR_VARIABLES map friendly variable names
			to GTA V XML timecycle keys.
	  • Helper functions:
		  - get_region_from_hash, get_region_from_name, get_region_name.
		  - get_weather_from_hash, get_weather_from_name, get_weather_name.
		  - Validation for names (is_region_name_valid, is_weather_name_valid).
	  • GTAVTimeCycle:
		  - Extends TimeCycle.
		  - Provides variable_from_xml_element to parse scalar values.
		  - Provides default_weather_cycle() with fallback defaults.
		  - Provides weather_cycle_from_xml_element() for structured parsing.
		  - load() reads and parses all weather XMLs and constructs the
			complete weather→region→cycle mapping.

	Purpose:
	--------
	  • Translate GTA V weather and region data into structured cycles for use
		by shaders and overlays.
	  • Allow blending and interpolation of environmental parameters based on
		weather, region, and time of day.
	  • Decouple raw game XMLs and hashes from the rendering logic.

	Notes:
	------
	  • Uses tinyxml2 for XML parsing.
	  • Supports multiple regions (currently GLOBAL and URBAN).
	  • If XML entries are missing, falls back to default weather cycles.
	  • Logs successful timecycle file loads through reshade::log.
	  • Weather transition and aurora handling occur in GTAVSource.cpp.
...
namespace GTAV
{
	...
	static const std::unordered_map<std::string_view, std::string_view> FLOAT_VARIABLES = {
		...
		{ "sky_hdr",						"sky_hdr" },
		{ "light_dir_mult",					"light_dir_mult" },
		{ "natural_ambient_multiplier",		"natural_ambient_multiplier" },
		{ "fog_density",					"fog_density" },
		{ "fog_base_height",				"fog_base_height" },
		{ "postfx_exposure",				"postfx_exposure" },
		{ "postfx_exposure_min",			"postfx_exposure_min" },
		{ "postfx_exposure_max",			"postfx_exposure_max" },
