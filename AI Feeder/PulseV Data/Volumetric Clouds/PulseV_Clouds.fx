...
// ============================================================================ 
//                      UI UNIFORMS (Cloud Preset Settings)
// ============================================================================

uniform int qualityPreset <
    string ui_category = "Cloud Preset Settings";
...
// ============================================================================ 
//                      UI UNIFORMS (Aurora Preset Settings)
// ===========================================================================


uniform int auroraQualityPreset <
    string ui_category = "Aurora Preset Settings";
    ui_category_closed = true;
    string ui_type = "combo";
    string ui_items = "Low\0Medium\0High\0Ultra\0";
> = 1;

static const int AURORA_SAMPLES_LOW   = 128;
static const int AURORA_SAMPLES_MED   = 256;
static const int AURORA_SAMPLES_HIGH  = 512;
static const int AURORA_SAMPLES_ULTRA = 1024;
// ============================================================================ 
//                      UI UNIFORMS (Cloud Settings)
// ============================================================================

uniform float cloudRenderDistance <
    string ui_category = "Cloud Settings";
    ui_category_closed = true;
    string ui_type = "drag";
    float ui_min = 10.0;
...
// ============================================================================ 
//                      UI UNIFORMS (Atmospheric Fog Settings)
// ============================================================================

uniform float3 atmosphericFogColor <
    string ui_category = "Atmospheric Fog";
    ui_category_closed = true;
    string ui_type = "color";
> = float3(0.52, 0.58, 0.67);
uniform float atmosphericFogDensity <
    string ui_category = "Atmospheric Fog";
    string ui_type = "drag";
    float ui_min = 0.0;
    float ui_max = 0.01;
    float ui_step = 0.00001;
> = 0.00045;
uniform float atmosphericFogHeightFalloff <
    string ui_category = "Atmospheric Fog";
    string ui_type = "drag";
    float ui_min = 0.0;
    float ui_max = 0.02;
    float ui_step = 0.0001;
> = 0.003;
...
// ============================================================================ 
//                      UI UNIFORMS (Moon Global Settings)
// ============================================================================

uniform float3 MoonColor <
    string ui_category = "Moon Settings";
    ui_category_closed = true;
string ui_type = "color";
...
uniform float cloudSoftParticleRange <
    string ui_category = "Depth Settings";
    string ui_type = "drag";
    float ui_min = 0.0;
    float ui_max = 50.0;
...
uniform float cloudSoftParticleBias <
    string ui_category = "Depth Settings";
    string ui_type = "drag";
    float ui_min = -5.0;
    float ui_max = 5.0;
...
texture3D NoiseTexture <source = "nubisnoise.dds";>
{
    Width = 128;
    Height = 128;
    Depth = 128;
...
texture3D CurlNoiseTexture <source = "curl3D.dds";>
{
    Width = 128;
    Height = 128;
    Depth = 256;
...
int getAuroraQualityPresetSamples()
{
    ...
    {
        case 0: return AURORA_SAMPLES_LOW;
        case 1: return AURORA_SAMPLES_MED;
        case 2: return AURORA_SAMPLES_HIGH;
        case 3: return AURORA_SAMPLES_ULTRA;
    }
    // Default to Medium if out of range
    return AURORA_SAMPLES_MED;
}
...
float4 renderClouds(float2 uv, LayerParameters bottomLayer, LayerParameters topLayer, int samples)
{
    ...
    float fogFactor = 0.0;
    if (atmosphericFogStrength > 0.0 && atmosphericFogDensity > 0.0)
    {
        float cameraBelow = max(0.0, height - ray.origin.y);
        float heightFactor = exp(-cameraBelow * atmosphericFogHeightFalloff);
        float distanceFactor = 1.0 - exp(-maxDistance * atmosphericFogDensity);
        fogFactor = saturate(distanceFactor * heightFactor * atmosphericFogStrength);
        finalColor = lerp(finalColor, atmosphericFogColor, fogFactor);
        alpha = saturate(alpha + fogFactor * (1.0 - alpha));
    
}
// Soft particle fade near geometry edges using scene depth (always applied if range > 0)
if (cloudSoftParticleRange > 0.0)
{
    float depthDiff = (depth - minDistance) - cloudSoftParticleBias;
    float softAlpha = saturate(depthDiff / max(1e-5, cloudSoftParticleRange));
    alpha *= softAlpha;
    finalColor *= softAlpha;
}

float alphaColor = saturate(alpha * cloudContrast);
finalColor = lerp(sky, finalColor, alphaColor);

    float alphaOut = saturate(alpha);
    float4 clouds = float4(finalColor, alphaOut);
...
#if !defined(PREBAKED_NOISE) || (PREBAKED_NOISE == 0)
