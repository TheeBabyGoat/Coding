...
static void inject_uniforms(effect_runtime* runtime, command_list* cmd_list, resource_view rtv, resource_view rtv_srgb)
{
	...
	if (enabled) {
		...
		stage_uniform("time_of_day_hr", (int)fmod(DataReader::get_time_of_day() * 24.0, 24.0));
		...
		const auto& wfrom = (GTAV::WeatherType)DataReader::get_from_weather_type();
		const auto& wto = (GTAV::WeatherType)DataReader::get_to_weather_type();

		const bool is_rainy = wfrom == GTAV::WeatherType::RAIN || wfrom == GTAV::WeatherType::THUNDER;
		const bool is_clearing = wto == GTAV::WeatherType::CLEARING;

		if (is_rainy) {
			stage_uniform("rain_value", 1.0f);
		}
		else if (is_clearing) {
			stage_uniform("rain_value", 1.0f - DataReader::get_weather_transition());
		}
		else {
			stage_uniform("rain_value", 0.0f);
		}
#elif defined RFX_GAME_RDR1
		const auto& wfrom = (RDR1::WeatherType)DataReader::get_from_weather_type();
		const auto& wto = (RDR1::WeatherType)DataReader::get_to_weather_type();

		const bool is_rainy = wfrom == RDR1::WeatherType::RAINY || wfrom == RDR1::WeatherType::STORMY;
		const bool is_clearing = wto == RDR1::WeatherType::FAIR || wto == RDR1::WeatherType::CLEAR;
