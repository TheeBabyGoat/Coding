...
namespace GTAV
{
	...
	const float GTAVSource::get_rain_value()
	{
		const bool is_rainy = get_weather_from() == WeatherType::RAIN || get_weather_from() == WeatherType::THUNDER;
		const bool is_clearing = get_weather_to() == WeatherType::CLEARING;

		if (is_rainy) {
			return 1.0f;
		}
		else if (is_clearing) {
			return 1.0f - weather_transition;
		}
		else {
			return 0.0f;
		}
