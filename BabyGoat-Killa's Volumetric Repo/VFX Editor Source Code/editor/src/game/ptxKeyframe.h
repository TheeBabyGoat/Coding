	...
	const char* lb4 = "Unnamed4")
{
	...
	if (ImGui::BeginTable(vfmt("KeyframeTable## kfProp {}", next_num()), num_columns + 1, ImGuiTableFlags_Borders))
	{
		//==========================================================================
		...
		for (size_t kf_idx = 0; kf_idx < keyframes.size(); kf_idx++)
		{
			...
			for (size_t columnIdx = 0; columnIdx < num_columns; columnIdx++)
			{
				...
				ImGui::DragFloat(vfmt("##Time kfProp {}", next_num()), &CurrEntry.vTime[columnIdx], 0.01);
			}
			...
			for (size_t columnIdx = 0; columnIdx < num_columns; columnIdx++)
			{
				...
				const char* vValue_Label = vfmt("##Value kfProp {}", next_num());
