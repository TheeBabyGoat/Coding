...
CloudsHandler::CloudsHandler()
{
	...
	char path[] = "common:/data/cloudkeyframes";
	bool has_loaded_from_file = false;

	fiStream file(path, "xml");

	if (file.is_opened())
	{
		std::string str = file.read_to<std::string>();

		auto vec = m_XmlParser.GetSettingsNames(str);
		if (vec.has_value())
		{
			fillCloudSettingsNamedVec(vec.value());
			has_loaded_from_file = true;
		}
	}
	
	if (!has_loaded_from_file)
	{
		LogInfo("Failed to open asset: {}.xml", path);
