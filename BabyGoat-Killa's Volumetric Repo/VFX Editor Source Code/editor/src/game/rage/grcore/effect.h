...
namespace rage
{
	...
	struct grcInstanceVar
	{
		...
		grcTexture* GetTexture() { return reinterpret_cast<grcTexture*>(Value); }
	...
	struct grcInstanceData
	{
		...
		grcInstanceData* GetMaterial() { return m_Flags & FLAG_MATERIAL ? this : m_Preset.Material; }
		const char* GetMaterialName() const { return (m_Flags & FLAG_MATERIAL) ? m_Preset.Name : m_Preset.Material ? m_Preset.Material->m_Preset.Name : nullptr; }
		bool isMaterial() const { return m_Flags & FLAG_MATERIAL; }
		bool isExtraData() const { return m_Flags & FLAG_EXTRA_DATA; }
		...
		Iterator GetIterator(int texMode = ITERATOR_TEX_MODE_INCLUDE) const
		{
			...
			default:
				LogInfo("grcInstanceData::GetIterator() -> Unknown tex mode {}", texMode);
				std::unreachable();
			}
		}
	};
