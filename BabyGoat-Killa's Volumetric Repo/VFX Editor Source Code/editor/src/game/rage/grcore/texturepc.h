...
#define FORMAT_A8R8G8B8 40
	...
	{
		enum Memory_t { SYSTEM, VIDEO, STAGING }; // System = Dynamic CPU Write Access, Staging = CPU Read Access
	...
	{
		grcTextureFactory() = default;
	public:

		static grcTextureFactory* GetInstance();

		//Can return nullptr
		grcTexture* CreateFromDDSImage(std::filesystem::path path, grcImage::Format img_format);
