...
#define verify_ptr(ptr) ptr ? ptr : "(NULL)"
	...
	if (ImGui::TreeNode(vfmt("Name: {} ## {}", treeNodeName, Packed_N)))
	{
		...
		case rage::PTXSHADERVAR_FLOAT:
		{
			...
			ImGui::DragFloat(vfmt("FloatV##{}", next_num()), (float*)&ptr->m_vector);
		...
		{
			auto* ptr = reinterpret_cast<ptxShaderVarVector*>(this);
			ImGui::DragFloat2(vfmt("Vector2##{}", next_num()), (float*)&ptr->m_vector, step, _min, _max);
			break;
		}
		case rage::PTXSHADERVAR_FLOAT3:
		{
			auto* ptr = reinterpret_cast<ptxShaderVarVector*>(this);
			ImGui::DragFloat3(vfmt("Vector3##{}", next_num()), (float*)&ptr->m_vector, step, _min, _max);
			break;
		}
		case rage::PTXSHADERVAR_FLOAT4:
		{
			auto* ptr = reinterpret_cast<ptxShaderVarVector*>(this);
			ImGui::DragFloat4(vfmt("Vector4##{}", next_num()), (float*)&ptr->m_vector, step, _min, _max);
			break;
		}
		...
		case rage::PTXSHADERVAR_TEXTURE:
		{
			...
			ImGui::Checkbox(vfmt("External Reference##{}", next_num()), &ptr->m_externalReference);
		...
		case rage::PTXSHADERVAR_KEYFRAME:
		{
			...
			ptxKeyframeTable(ptr->m_keyframe, PTX_KF_FLOAT4);
	...
	//ImGui::SameLine();
	//if (ImGui::Button(vfmt("Copy to clipboard## {}", next_num())))
	//{
