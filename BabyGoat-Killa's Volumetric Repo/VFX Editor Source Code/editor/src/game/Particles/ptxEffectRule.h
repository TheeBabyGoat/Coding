...
namespace rage
{
	...
	struct alignas(16) ptxEffectRule : public pgBaseRefCounted
	{
		...
		atConstString m_name;								// the name of the effect rule
		datRef<ptxFxList> m_Self_FxList;					// pointer to the effect list where we have loaded from

		// timeline and evolution data
		ptxTimeLine m_timeline;
		datOwner<ptxEvolutionList> m_pEvolutionList;
		...
		rage::Vector4 m_viewportCullingSphereOffset;		//vec3 // the offset of the viewport culling sphere
		...
		PtFxColnType m_colnType;										// the type of collision that this effect has	
