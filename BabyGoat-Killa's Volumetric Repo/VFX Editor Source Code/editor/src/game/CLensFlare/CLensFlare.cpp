...
void LensFlares_DebugOverlay::DrawOverlay()
{
	...
	for (auto& data : m_DrawData)
	{
		...
		if (data.thickness != self->m_DefaultThickness)
			ImGui::GetBackgroundDrawList()->AddCircleFilled(data.pos, 3, ConvertToImguiCol32(data.col),16);
	}
}
...
CFlareFX& CFlareFX::operator=(const CFlareFX& other)
{
	if (this == &other)
		return *this;
	memcpy(this, &other, sizeof(CFlareFX));
	return *this;
}
