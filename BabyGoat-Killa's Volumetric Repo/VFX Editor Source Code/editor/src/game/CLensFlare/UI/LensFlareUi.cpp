	...
	if (ImGui::BeginTable("##FlaresTable", 3))
	{
		...
		}
		{
			if (ImGui::BeginPopup("SettingsToggle"))
			{
				...
				ImGui::Checkbox("Draw Flares", &m_Handler.pCLensFlares->m_bEnabled);
		...
		for (size_t i = 0; i < std::size(ptr->m_pTextures); i++)
		{
			...
			if (ImGui::TreeNode(vfmt(" {} Texture##{}", TextureNames[i], i)))
			{
				...
				if (ImGui::BeginPopupContextItem("dds texture replace popup"))
				{
					...
					{
						texture = rage::grcTextureFactory::GetInstance()->CreateFromDDSImage(curr_buff, rage::grcImage::Format::DXT1);
					}
					if (ImGui::Selectable("DXT3"))
					{
						texture = rage::grcTextureFactory::GetInstance()->CreateFromDDSImage(curr_buff, rage::grcImage::Format::DXT3);
					}
					if (ImGui::Selectable("DXT5"))
					{
						texture = rage::grcTextureFactory::GetInstance()->CreateFromDDSImage(curr_buff, rage::grcImage::Format::DXT5);
					}
