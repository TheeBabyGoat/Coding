...
static uint32_t render_target_width = 0;
static uint32_t render_target_height = 0;
...
void on_init_swapchain(reshade::api::swapchain* swapchain, bool resize)
{
    ...
    const auto& back_buffer =
        swapchain->get_device()->get_resource_desc(swapchain->get_current_back_buffer());

    if (static_cast<int>(back_buffer.usage & reshade::api::resource_usage::render_target) == 0) {
        return;
    }

    LOG(("Detected resolution: " + std::to_string(back_buffer.texture.width) +
        "x" + std::to_string(back_buffer.texture.height)).c_str());

    render_target_width = back_buffer.texture.width;
    render_target_height = back_buffer.texture.height;
...
uint32_t get_render_target_width() {
    return render_target_width;
}

uint32_t get_render_target_height() {
